...
#if defined(NDEBUG)
# error "Cryptoescudo cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x070390081e6d136d5714b9395d83807d0b2859f98e49bb2fcd995467b6b65016");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptoescudo: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cryptoescudo Signed Message:\n";
...
{
    // Cryptoescudo: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 60 * 1;
...
unsigned int static GetNextWorkRequired_STD(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptoescudo: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 2.0 * 60; // 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
        	...
        	if (pindexLast->nHeight+1 >= 31) { DiffMode = 2; }
        }
        if (DiffMode == 1) { return GetNextWorkRequired_STD(pindexLast, pblock); } //legacy diff mode
        else if (DiffMode == 2) { return GetNextWorkRequired_KGW(pindexLast, pblock); } // KGW
        return GetNextWorkRequired_KGW(pindexLast, pblock); // KGW
}
...
{
    // Cryptoescudo: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb1afa88bdc441ff0908d8b49c41fe347b6a2e4ad9200e6024abd54a2dc2a46de");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "1143.1385.1640.1910.1974.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1395763200;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 181610;

        if (fTestNet)
        {
            block.nTime    = 1394064000;
            block.nNonce   = 181610;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd7ff1a382b361275b2a09af0af3ca2427ef79c5e1cae91288f829395e21364ea"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cryptoescudo: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (string(pfrom->cleanSubVer.c_str()) == "/Escudeiro:0.9.2/")
           {
            // disconnect from peers with this initial version
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoescudoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoescudoMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoescudoMiner(CWallet *pwallet)
{
    printf("CryptoescudoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptoescudo-miner");
    ...
    try { loop {
        ...
        printf("Running CryptoescudoMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoescudoMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoescudoMiner, pwallet));
}
